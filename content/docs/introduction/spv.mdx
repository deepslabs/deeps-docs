---
title: SPV
description: How SPV works in the DeepS network
---

Simplified Payment Verification (SPV) is a critical part of the DeepS security model. It prevents signing operations from relying on fraudulent source-chain data.

An SPV service registers on-chain through remote attestation and runs multiple blockchain light clients inside an SGX enclave. It provides transaction-verification services to the monitor.

The service signs each response with an SGX-protected key. The monitor verifies the signature against the SPV public key registered on-chain, confirming that the response originated from an authentic enclave running approved code.
...
Public mode provides a common reference for cross-validation and synchronization checks.

Local mode decentralizes verification sources and reduces reliance on a single service.

Each light client is based on the corresponding blockchain's open-source implementation and follows that network's verification protocol, such as Bitcoin SPV or the Ethereum light-client protocol.
...
## Chain-Specific Verification Schemes

**1. Bitcoin Simplified Payment Verification**
