---
title: RingVRF
description: The Ring VRF algorithm used in DeepS for anonymous committee selection
---
...
At the same time, committee-member identities must remain confidential to reduce the risk of collusion and targeted attacks against the signing protocol.
...
> **In essence**: Ring VRF allows a user to prove that their public key belongs to the set \{pk₀,...,pkₙ₋₁\} and to generate a verifiable random output `v` with private key `sk`, while **hiding the index of their public key** `ℓ`.

DeepS uses the **Ring VRF algorithm** to satisfy these requirements.

The algorithm allows a user to prove that the public key corresponding to their private key belongs to a public-key set **without revealing which key is theirs**.

$$
RVRF = \{((pk_0, \ldots, pk_{N-1}, r, v), sk, \ell) \mid \ell \in \{0, \ldots, N-1\} \text{ s.t. } (pk_\ell, sk) \in R_{pk} \wedge v \leftarrow PRF_{sk}(r)\}
$$

The algorithm takes a **public set of public keys** and a **random seed** as inputs.

A Pedersen commitment is used as the commitment scheme.
